...
| `wifi-densepose-tactical` | Tactical person-localization for rescue operations (hostage rescue, barricade). Decision-support bounded context over `wifi-densepose-mat`: `Structure → Room → sensor-placement` model, `TacticalEngine` (per-room ingest → MAT RSSI point-fix or room-level presence → tracked anonymous `PersonContact`s → live `TacticalPicture`), `EntryAdvisor` (Focus/Active/Monitor/NoContact ordering — advisory only), Axum JSON+WS server (`ruview-tactical` binary) with a self-contained live floor-plan dashboard, a hardware-free scenario simulator, an Android APK shell (JNI + WebView, `android/`) with an auxiliary BLE device-presence overlay, and `ruview-csi-bridge` (feature `bridge`) — a host-side UDP aggregator decoding ESP32 ADR-018 CSI → `/api/csi`. Presence + coarse location only — **cannot** distinguish hostage from suspect, confirm a weapon, or prove a room is empty; no validated tactical accuracy, `sim` output is synthetic. Depends on MAT with `default-features = false` (no ONNX). |
